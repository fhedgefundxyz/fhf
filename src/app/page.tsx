    ...
    <>
      <section className="min-h-screen flex flex-col justify-center px-6 md:px-10 max-w-[1280px] mx-auto">
        <Reveal>
          <div className="inline-flex items-center gap-2 rounded-full px-3.5 py-[5px] pl-2.5 mb-10 w-fit"
            ...
            <span className="font-mono text-[10.5px] text-[#C8FF00] tracking-[0.1em] uppercase font-medium">Live on Base Network</span>
          </div>
        </Reveal>
        ...
        <Reveal delay={100}>
          <h1 className="font-serif font-normal leading-[0.92] tracking-[-0.035em] text-[#E8E6E3] max-w-[900px] mb-8"
            ...
            <span className="text-[#C8FF00] italic">Flaunch&apos;s</span> best
          </h1>
        </Reveal>
        ...
        <Reveal delay={300}>
          <div className="flex gap-3.5 flex-wrap">
            <Link href="/dashboard" className="bg-[#C8FF00] text-[#050505] px-8 py-[15px] rounded-[10px] font-mono text-xs font-semibold tracking-[0.04em] uppercase no-underline transition-transform hover:-translate-y-px">
              Explore Fund →
            </Link>
            <Link href="/stake" className="px-8 py-[15px] rounded-[10px] font-mono text-xs text-[#E8E6E3] tracking-[0.04em] uppercase no-underline transition-colors hover:border-[rgba(200,255,0,0.2)]"
      ...
      <section className="max-w-[1280px] mx-auto px-6 md:px-10 py-[140px] pb-[120px]" style={{ borderTop: '1px solid var(--border)' }}>
        <Reveal>
          <div className="flex flex-col md:flex-row justify-between md:items-end mb-[72px] gap-6">
            <div>
              <div className="font-mono text-[10px] text-[#C8FF00] tracking-[0.14em] uppercase mb-4">Mechanism</div>
          ...
          {[
            ...
            <Reveal key={i} delay={i * 100}>
              <div className="card-hover bg-[#0A0A0A] p-9 md:p-[56px_44px]">
                <div className="font-serif text-[28px] text-[#C8FF00] italic mb-7">{v.icon}</div>
      ...
      <section className="max-w-[1280px] mx-auto px-6 md:px-10 py-[100px] pb-20 text-center" style={{ borderTop: '1px solid var(--border)' }}>
        <Reveal>
          <h2 className="font-serif font-normal text-[#E8E6E3] tracking-[-0.03em] leading-[1.05] mb-5" style={{ fontSize: 'clamp(36px, 5vw, 60px)' }}>
            Stop guessing.<br/><span className="text-[#C8FF00] italic">Start earning.</span>
          </h2>
          <p className="font-body text-lg font-light mb-10" style={{ color: 'var(--tx-muted)' }}>One token. Managed exposure. Real yield.</p>
          <Link href="/stake" className="inline-block bg-[#C8FF00] text-[#050505] px-10 py-4 rounded-[10px] font-mono text-[13px] font-semibold tracking-[0.04em] uppercase no-underline transition-transform hover:-translate-y-px">
            Buy FHF on Flaunch
          </Link>
        </Reveal>
      </section>
      ...
      <footer className="max-w-[1280px] mx-auto px-6 md:px-10 py-8 flex flex-col md:flex-row justify-between items-center gap-4" style={{ borderTop: '1px solid var(--border)' }}>
        ...
        <div className="flex gap-6">
          {['Twitter','Telegram','Docs','Basescan'].map(l => (
            <a key={l} href="#" className="font-mono text-[10px] no-underline tracking-[0.06em] uppercase transition-colors hover:text-[#C8FF00]" style={{ color: 'var(--tx-dim)' }}>{l}</a>
          ))}
        </div>
      </footer>
    </>
